...
// Resume autoplaying clips after a backgrounded tab is reopened without a refresh.
// iOS suspends them and never restarts them, leaving the tour clip frozen on a
// half-rendered frame. play() only, never load(): load() resets the element and
// leaves it painting nothing but its background.
(function initVideoResume() {
    function reviveAll() {
        Array.prototype.forEach.call(document.querySelectorAll('video'), function (v) {
            if (!v.paused || v.hasAttribute('controls')) return;
            var p = v.play();
            if (p && p.catch) {
                p.catch(function () { try { v.currentTime = 0; } catch (e) {} });
            }
        });
    }
    ...
    window.addEventListener('pageshow', function (e) { if (e.persisted) reviveAll(); });
}());
