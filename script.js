...
(function initLiveReviews() {
    ...
    // every height, then read every height, then pin every height. Both grids go
    // through together for the same reason — the week list is its own grid with
    // its own column width, so it gets its own tallest, but measuring it in the
    // same pass keeps the page to one flush instead of two.
    function measureAndPin() {
        // A hidden grid measures every card at zero, so it would pin the lot to
        // 0px and hand back collapsed cards the next time it's shown.
        var lists = [el.list];
        if (!el.week.hidden && !el.weekReviews.hidden) lists.push(el.weekList);
        var groups = [];
        lists.forEach(function (container) {
            if (!container) return;
            var live = [];
            container.querySelectorAll('.rv-card').forEach(function (c) {
                if (c.dataset.rvOpen) return;
                c.style.height = 'auto';
                live.push(c);
            });
            if (live.length) groups.push({ cards: live, tallest: 0 });
        });
        if (!groups.length) return;
        groups.forEach(function (g) {
            g.cards.forEach(function (c) { g.tallest = Math.max(g.tallest, c.offsetHeight); });
        });
        groups.forEach(function (g) {
            g.cards.forEach(function (c) { c.style.height = g.tallest + 'px'; });
        });
    }
    ...
    function buildWeek() {
        ...
        // where the week's ratings actually came from. A storefront's first
        // rating arrives as a 'first' event with no 'from' field: count it
        // from zero rather than dropping it.
        //
        // Deltas are signed and must stay signed. Apple drops ratings as well
        // as adding them, which arrives as a negative delta (6 -> 5). Clamping
        // each event at zero counted the add and swallowed the matching
        // removal, so the week read one rating higher than the tracker and a
        // storefront that ended the week where it started still showed a '+1'.
        // Net per storefront first, then keep only the ones that finished up.
        var byCountry = {};
        (state.events || []).forEach(function (ev) {
            var at = new Date(ev.at).getTime();
            if (at < since || typeof ev.to !== 'number') return;
            if (ev.type !== 'delta' && ev.type !== 'first') return;
            if (state.cc !== 'all' && ev.cc !== state.cc) return;
            byCountry[ev.cc] = (byCountry[ev.cc] || 0) +
                (ev.to - (typeof ev.from === 'number' ? ev.from : 0));
        });

        var newRatings = Object.keys(byCountry).reduce(function (sum, cc) {
            return sum + byCountry[cc];
        }, 0);

        el.weekNum.textContent = Math.max(0, newRatings).toLocaleString(lang);
        el.weekNum.classList.toggle('is-up', newRatings > 0);

        el.weekCountries.textContent = '';
        if (state.cc !== 'all') byCountry = {};
        Object.keys(byCountry).filter(function (cc) {
    ...
    function load() {
        ...
        Promise.all([
            ...
            // A review flagged `removed` has left Apple's feed and its star has left
            // the count; the tracker keeps the record, this page drops it.
            state.reviews = (reviews || []).filter(function (r) { return r && r.cc && r.rating && !r.removed; });
            state.latest = latest;
