...
- **Translation depth varies**:
  ...
  - `zh` — shorter FAQ section, no FAQ JSON-LD schema at all (different structure from the other 14).
...
2. Mirror in each language's `index.html` with translated content. Watch out for `zh` (no JSON-LD entry to add) and the 11 partially-translated languages (their existing later FAQs may still be in English).
