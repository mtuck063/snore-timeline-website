...
│
├── da/  de/  es/  fr/  ja/  ko/  nl/  no/   # Translated language directories
└── pl/  ru/  sv/  th/  tr/  zh/             # (each has its own index.html, privacy.html, 404.html)
```

## Internationalization (i18n)

The site is published in **15 languages**. The English version lives at the root; each other language has its own directory containing `index.html`, `privacy.html`, and `404.html`.
...
- **Asset paths**: Translated pages use `../` prefix for CSS, JS, images, video, and favicons.
- **Hreflang**: All pages include `<link rel="alternate" hreflang="...">` tags for every language plus `x-default`. The sitemap mirrors this with `xhtml:link` annotations.
- **Language switcher**: Footer dropdown using `.lang-switcher` / `.lang-dropdown` classes (CSS in `styles.css`).
- **Brand name**: "Snore Timeline" stays untranslated in every language.
- **Testimonials**: Kept in their original language (not translated).
- **Translation depth varies**:
  - `en`, `fr`, `es` — fully translated including the FAQ JSON-LD schema.
  - `da`, `de`, `ja`, `ko`, `nl`, `no`, `pl`, `ru`, `sv`, `th`, `tr` — FAQ section is translated through the most recent entry. Older sections of these files may still have partially-translated areas (verify before assuming).
  - `zh` — brought to full FAQ parity on 2026-08-01; it now has the same 9 landing FAQ items and its own FAQPage JSON-LD, generated from its published Chinese text.

### When adding a new FAQ

1. Add to root `index.html`: JSON-LD entry under the `mainEntity` array AND the visible `<div class="faq-item">` block.
2. Mirror in each language's `index.html` with translated content. Every language now carries the FAQ JSON-LD, `zh` included, so each one needs both the visible block and the schema entry. Keep the schema `text` identical to the visible answer paragraphs joined with a space, excluding the `.faq-learn-more` link — that is the convention across all 23 locales.
